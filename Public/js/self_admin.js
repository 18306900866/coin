...
$(window).load(function () {
    function ajax(page,start,end,ip,operate,admin,controller) {
        var arr = {'page': page, "start": start,"end":end, "ip": ip, "operate": operate, "admin": admin};
        $.ajax({
    ...
    $(".search").on("click", function () {
        ...
        var admin =$(".self_name").text()+",";
        var controller="record_page";
        if (start != '' || ip != '' || operate != '' ||end!=''){
    ...
    $(document).on("click", ".daochu", function () {
        ...
        if (time == ',' && ip == '' && operate == '' && admin == '') {
            ...
            window.location.href = "output?time=" + time + "&ip=" + ip + "&operate=" + operate + "&admin=" + admin + "&timestamp=" + timestamp ;
        }
    })
})