	...
	<body>
    	<div class="panel admin-panel">
			<div class="panel-head">
				...
				<strong class="zijing" name="miao">彩种图片</strong>
			</div>
    	</div>
		<div class="tabale_txjl" style="min-height: 470px"  name="miaoo">
			<table class="table lo_sort lo_img">
				<thead>
					<td>对象名</td>
					<td>图片预览</td>
					<td>跳转地址</td>
		...
		//复制表格数据
		function tdshow(val){
		var tddd = "<td>"+val.obj+"</td>"+
					"<td><img style='width: 60px' src='"+val.img+"'></td>";

					if(val.types==1){
						tddd+="<td><input type='text' class='lo_sort_in pagenum' value='"+val.url+"' data-sortid='"+val.id+"'></td>";
					}else{
						tddd+="<td>无操作</td>";
					}

					tddd += "<td><input type='text' class='lo_sort_in pagenum' value='"+val.img+"' data-sortid='"+val.id+"'></td>";

					if(val.state==1){
						tddd+="<td><select name='state'><option value ='1' selected = 'selected'>已开启</option><option value ='0'>已关闭</option></select></td>";
					}else{
						tddd+="<td><select name='state'><option value ='1'>已开启</option><option value ='0'  selected = 'selected'>已关闭</option></select></td>";
					}

					tddd += "<td><button class='button border-blue'>图片修改</button></td>";
		...
		function ssc(a){
			...
			var state = a.eq(4).children('select').eq(0).val();
			var url = '';
			var id = a.eq(3).children('input').attr('data-sortid');
			if(a.eq(2).text()!='无操作'){
				url = a.eq(2).children('input').val();
			}
			var img = a.eq(3).children('input').val();
			var bb = a.eq(0).text();

			// alert(a.eq(1).children('img').attr('src'));
			...
			if(confirm('你确定要修改吗？')){
				...
				a.eq(1).children('img').attr('src',img);
			}else{
				dataa = {
					id:'',
					img:''
